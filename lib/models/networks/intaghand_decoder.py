...
from lib.utils.utils import projection_batch #, get_dense_color_path, get_graph_dict_path, get_upsample_path
# from models.model_zoo import GCN_vert_convert, graph_upsample, graph_avg_pool
from lib.models.networks.model_attn.DualGraph import DualGraph

IMG_SIZE = 384 #320 #384 #256*2

def get_dense_color_path():
    dense_path = os.path.join(os.path.dirname(__file__), 'gcn_core/v_color.pkl')   
    return dense_path 

def get_graph_dict_path():
    mano_path = {'left': os.path.join(os.path.dirname(__file__), 'gcn_core/graph_left.pkl'),
                'right': os.path.join(os.path.dirname(__file__), 'gcn_core/graph_right.pkl')}   
    return mano_path 

def get_upsample_path():
    upsample_path = os.path.join(os.path.dirname(__file__), 'gcn_core/upsample.pkl')     
...
class decoder(nn.Module):
    def __init__(self,
        ...
        self.root_head = nn.Linear(self.gcn_out_dim[-1], 3)
    ...
    def forward(self, global_feature_left, global_feature_right, fmaps):
    # def forward(self, x, fmaps):
        if False:
            ...
            assert global_feature_left.shape[1] == self.gf_dim
            assert global_feature_right.shape[1] == self.gf_dim
            fmaps = fmaps[:-1]
            bs = global_feature_left.shape[0]

            pel, per = self.get_hand_pe(bs, num=self.vNum_in)
            Lf = torch.cat([self.gf_layer_left(global_feature_left).unsqueeze(1).repeat(1, self.vNum_in, 1), pel], dim=-1)
            Rf = torch.cat([self.gf_layer_right(global_feature_right).unsqueeze(1).repeat(1, self.vNum_in, 1), per], dim=-1)
        ...
        params = self.params_head(temp)
        root_left = self.root_head(temp)
        scale['left'] = params[:, 0]
        trans2d['left'] = params[:, 1:]
        root['left'] = root_left[:]
        temp = self.avg_head(Rf.transpose(-1, -2))[..., 0]
        params = self.params_head(temp)
        root_right = self.root_head(temp)
        scale['right'] = params[:, 0]
        trans2d['right'] = params[:, 1:]
        root['right'] = root_right[:]

        handDictList = []

        paramsDict = {'scale': scale, 'trans2d': trans2d, 'root': root}
...
def load_decoder(cfg, encoder_info):
    ...
    model = decoder(
        ...
        f_out_Dim=cfg.IMG_DIMS,
        gcn_in_dim=cfg.GCN_IN_DIM,
        gcn_out_dim=cfg.GCN_OUT_DIM,
        graph_k=cfg.graph_k,
        graph_layer_num=cfg.graph_layer_num,
        vertex_num=778,
