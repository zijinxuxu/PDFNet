...
class ManoRender(nn.Module):
  def __init__(self, opt):
    ...
    self.MANO_GT = ManoModel(model_path=self.rhm_path,
                  num_pca_comps=n_comps,
                  flat_hand_mean=True,   # False for FreiHAND and True for HO3D
                  use_pca=opt.using_pca) 
    ...
    self.register_buffer('weighted_lms', torch.from_numpy(weight.reshape([1, 21, 2])))
  ...
  def Split_coeff(self, theta, index, K):
    if self.opt.using_pca:
      ...
      betas_coeff_l =  theta[:, 48:58] *0# TEST with shape   
      ...
      betas_coeff_r =  theta[:, 109:119] *0# TEST with shape
    ...
    w, h = self.input_res, self.input_res
    ret_global_transl_coeff_r_up_x = global_transl_coeff_r_up[:, 2] * (global_transl_coeff_r_up[:, 0] + cx - cw) / fx
    ret_global_transl_coeff_r_up_y = global_transl_coeff_r_up[:, 2] * (global_transl_coeff_r_up[:, 1] + cy - ch) / fy
    ...
    ret_global_transl_coeff_l_up_x = global_transl_coeff_l_up[:, 2] * (global_transl_coeff_l_up[:, 0] + cx - cw) / fx
    ret_global_transl_coeff_l_up_y = global_transl_coeff_l_up[:, 2] * (global_transl_coeff_l_up[:, 1] + cy - ch) / fy
  ...
  def get_uv_root_3d(self, index, off_hm_pred, depth_pred, K):
    fx = K[:,0,0]
    fy = K[:,1,1] 
    cw = K[:,0,2]
    ch = K[:,1,2]
    ...
    cy = (index // (self.input_res // self.opt.down_ratio)) * self.opt.down_ratio
    w, h = self.input_res, self.input_res
    root_x = depth_pred * (off_hm_pred[:, 0] + cx.squeeze() - cw) / (fx+1e-7)
    root_y = depth_pred * (off_hm_pred[:, 1] + cy.squeeze() - ch) / (fy+1e-7)
    root_xyz = torch.stack((root_x, root_y, depth_pred),1).unsqueeze(1)
