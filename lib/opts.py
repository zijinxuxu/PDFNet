...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--depth', action='store_true',
                             help='whether use extra depth feature')                                                      
    # system
    ...
    self.parser.add_argument('--lr_step', type=str, default='30',
    ...
    self.parser.add_argument('--center_weight', type=float, default=200.,
                             help='loss weight for center heatmaps.')
    self.parser.add_argument('--heatmaps_weight', type=float, default=10.,
                             help='loss weight for keypoint heatmaps.')  
    self.parser.add_argument('--handtype_weight', type=float, default=200.,
                             help='loss weight for handtype.')  

    self.parser.add_argument('--off_weight', type=float, default=100,
                             help='loss weight for keypoint local offsets.')
    self.parser.add_argument('--wh_weight', type=float, default=20,
    ...
    self.parser.add_argument('--reproj_weight', type=float, default=1,
    ...
    self.parser.add_argument('--bone_dir_weight', type=float, default=200.,
                             help='bone_dir_weight for bone dir loss')    
    self.parser.add_argument('--joints_weight', type=float, default=1,
                             help='joints_weight for 3d joints loss') 
    self.parser.add_argument('--mano_weight', type=float, default=1,
    ...
    # ------------for pointnet params--------
    self.parser.add_argument('--size', type=str, default='full', help='how many samples do we load: small | full')
    self.parser.add_argument('--SAMPLE_NUM', type=int, default = 1024,  help='number of sample points')
    self.parser.add_argument('--JOINT_NUM', type=int, default = 21,  help='number of joints')
    self.parser.add_argument('--INPUT_FEATURE_NUM', type=int, default = 3,  help='number of input point features')
    self.parser.add_argument('--PCA_SZ', type=int, default = 63,  help='number of PCA components') # i just use joints position.
    self.parser.add_argument('--knn_K', type=int, default = 64,  help='K for knn search')
    self.parser.add_argument('--sample_num_level1', type=int, default = 512,  help='number of first layer groups')
    self.parser.add_argument('--sample_num_level2', type=int, default = 128,  help='number of second layer groups')
    self.parser.add_argument('--ball_radius', type=float, default=0.015, help='square of radius for ball query in level 1')
    self.parser.add_argument('--ball_radius2', type=float, default=0.04, help='square of radius for ball query in level 2')
    self.parser.add_argument('--sample_strategy', type=str, default='random', help='how to sample: random | FPS')

    # ---------------for IntagHand GCN params---------------
    self.parser.add_argument('--DECONV_DIMS', type=str, default='256,256,256,256', help='deconv_dims')
    self.parser.add_argument('--GCN_IN_DIM', type=str, default='512,256,128', help='GCN_IN_DIM')
    self.parser.add_argument('--GCN_OUT_DIM', type=str, default='256,128,64', help='GCN_OUT_DIM')
    self.parser.add_argument('--IMG_DIMS', type=str, default='256,128,64', help='IMG_DIMS')
    self.parser.add_argument('--graph_k', type=int, default=2, help='graph_k')
    self.parser.add_argument('--graph_layer_num', type=int, default=4, help='graph_layer_num')
  ...
  def parse(self, args=''):
    ...
    opt.DECONV_DIMS = [int(i) for i in opt.DECONV_DIMS.split(',')]
    opt.GCN_IN_DIM = [int(i) for i in opt.GCN_IN_DIM.split(',')]
    opt.GCN_OUT_DIM = [int(i) for i in opt.GCN_OUT_DIM.split(',')]
    opt.IMG_DIMS = [int(i) for i in opt.IMG_DIMS.split(',')]
  ...
  def update_dataset_info_and_set_heads(opt, dataset):
    ...
    if opt.task in ['simplified', 'artificial','interact']:
      opt.heads = {'hm': 2} # left/right
      opt.heads.update({'wh':2})
      if opt.reproj_loss:
        opt.heads.update({'params': (61)*2}) # 3 orient + 45 pose + 10 shape + 3 trans 
      ...
      if opt.off:
        opt.heads.update({'off_hm': 2})
        opt.heads.update({'off_lms': 21*2})
    else:
